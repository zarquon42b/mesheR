gaussDisplace <- function(mesh1,mesh2,sigma,gamma=2,W0,f,oneway=F,k=1,nh=NULL,tol=0,cores=detectCores(),...)
{
  out <- NULL
  t0 <- Sys.time()
  sigma0 <- sigma
  ...
  S0 <- t(mesh1$vb[1:3,])
  sigma <- (sigma0*f^(-k))^2
  S <- vert2points(vcgClost(mesh1,mesh2,sign=F))
  t1 <- Sys.time()
   ...
   if (oneway)
  ...
  else
    {
      ...
      M <- vert2points(vcgClost(mesh2,mesh1,sign=F))
    }
    t2 <- Sys.time()
  if (!is.null (nh))
    {
...
gaussDisplMesh3d <- function(mesh1,mesh2,iterations=10,smooth=NULL,smoothit=10,smoothtype=c("taubin","laplace","HClaplace"),sigma=20,gamma=2,f=1.2,oneway=F,k=1,tol=0,lm1=NULL,lm2=NULL,icp=FALSE,icpiter=3,uprange=0.95,rhotol=1,nh=50,toldist=0,patch=NULL,repro=FALSE,cores=detectCores(),...)
  {
    if (!is.null(patch))
    ...
    for (i in 1:iterations)
      {
        ...
        if (!is.null(smooth))
          { if (i %% smooth == 0)
            ...
            tmp <- gaussDisplace(mesh1,mesh2,sigma=sigma,gamma=gamma,f=f,W0=vert2points(mesh1),nh=nh,k=i,tol=toldist,cores=cores)
