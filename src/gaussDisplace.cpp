...
rowvec relax_pt(mat D1, mat D2,rowvec distance1, rowvec distance2, double sigma, double gamma, bool oneway, vec dif1, vec dif2) {
  ...
  for (uint i = 0; i < distance1.size(); i++) {
    float g1tmp = smooth_weight(distance1(i),sigma);
  ...
  if (!oneway) {
    for (uint i = 0; i < distance2.size(); i++) {
      float g2tmp = smooth_weight(distance2(i),sigma);
...
RcppExport SEXP displaceGauss(SEXP iomat_, SEXP D1_, SEXP D2_, SEXP sigma_, SEXP gamma_, SEXP clIW_, SEXP clIWdistances_, SEXP clIP_, SEXP clIPdistances_, SEXP tol_, SEXP rt0_, SEXP rt1_, SEXP rc_, SEXP oneway_,SEXP threads_= wrap(1)){
  try {
...
#pragma omp parallel for schedule(static)
  for (uint i = 0; i < out.n_rows; i++) {
    ...
    out.row(i) = relax_pt(armaD1.rows(tmpW),armaD2.rows(tmpP),clIWdistances.row(i), clIPdistances.row(i),sigma, gamma, oneway,diff1(tmpW),diff2(tmpP));
